...
async fn execution_proxy_handler(
    ...
    for rpc in rpcs {
        let req = state.http_client.post(&rpc)
    ...
    Response::builder()
        .status(StatusCode::BAD_GATEWAY)
        .body(axum::body::Body::from("All configured execution RPCs failed"))
        .unwrap()
}

async fn consensus_proxy_handler(
    State(state): State<AppState>,
    req: Request,
) -> Response {
    let rpcs = {
    ...
    let proxy_path = original_path.strip_prefix("/consensus").unwrap_or(original_path);
    ...
    Response::builder()
        .status(StatusCode::BAD_GATEWAY)
        .body(axum::body::Body::from("All configured consensus RPCs failed"))
        .unwrap()
}